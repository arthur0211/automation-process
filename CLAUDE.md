...
- **Extension** (All phases complete): Captures user actions, screenshots, video; stores in IndexedDB; exports to JSON/HTML/Markdown/Playwright/Cypress/Selenium/Puppeteer/PDF/GitHub Issues; centralized Zustand state; session management; multi-tab recording; video playback; JSON import
...
```bash
cd extension
npx vitest run           # run all tests (341 tests, vitest + happy-dom + fake-indexeddb)
...
- `lib/export/puppeteer-exporter.ts` — export to Puppeteer .js test with page API and Promise.all navigation
